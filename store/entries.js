...
export const mutations = {
  ...
  setSlug (state, payload) {
    state.data[payload.key].slugEntry = payload.fields
  },
...
export const actions = {
  ...
  async getEntries ({ commit, rootState }, data) {
    try {
      commit('setLoading', { key: data.content_type, loading: true })
      ...
      commit('setEntries', { key: data.content_type, items })
    } catch (e) {
      commit('setError', { key: data.content_type, error: { statusCode: 404, message: 'Could not fetch entries.' } })
    } finally {
      commit('setLoading', { key: data.content_type, loading: false })
    }
  }
}
